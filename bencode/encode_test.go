...
func TestEncode(t *testing.T) {
	...
	var encodeCases = []testCase{
		...
		//tag is ","
		{eT2{A: "foo", B: "bar"}, `d1:A3:foo1:B3:bare`, false},
		...
		{
			eT4{Time: myTimeType{now}, Foo: myBoolType(true)},
			fmt.Sprintf("d1:f1:y1:ti%dee", now.Unix()),
			false,
		},
		{ // an error will be returned if a child can't be marshalled
			eT4WithErrorChild{Name: "Foo", Error: errorMarshalType{}},
			"", true,
		},
		...
		{
			et5{Answer: 42, Foo: myBoolTextType(true), Name: "Nova"},
			`d1:ai42e1:f1:y1:n4:Novae`,
			false,
		},
		{ // an error will be returned if a child TextMarshaler returns an error
			eT5WithErrorChild{Name: "Foo", Error: errorTextMarshalType{}},
			"", true,
		},
		...
		{
			&eT6{X: "x", Time: &myTimeType{now}, Foo: myBoolPtrType(true), Y: "y"},
	...
	for i, tt := range encodeCases {
		...
		if tt.out != string(data) && !tt.err {
			t.Fatalf("#%d: Val: %q != %q", i, data, tt.out)
		}
	}
}
